...
public class PhotoHostAuthFactory<T> extends AuthFactory<String, T>  {
	private static final Logger LOGGER = LoggerFactory.getLogger(PhotoHostAuthFactory.class);
	...
	public AuthFactory<String, T> clone(boolean required) {
		return new PhotoHostAuthFactory<>(required, authenticator(), this.realm, this.generatedClass);
	}
			...
			try {
				if (cookies != null ) {
					...
					if (cookies.length == 1) {
						...
						System.out.println("-------------------------------- fabrica says в массиве больше одного куки. нужно разобрать массив. tokenId : " + tokenId);
					}
					// декодируем
					// (пока непонятно когда и кем идет кодирование, поэтому выключаю декодирование)
